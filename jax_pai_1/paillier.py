...
SUPPORT_TYPE = ["float", "int", "numpy.ndarray",'DeviceArray']
...
class PaillierPublicKey(object):
    ...
    def raw_encrypt(self, plaintext, r_value=None):
        ...
        """
        if isinstance(plaintext,DeviceArray):
            plaintext = plaintext._value
    ...
    def encrypt(self, arr, is_pool=False):
        ...
        """
        if isinstance(arr, np.ndarray) or isinstance(arr, float) or \
                isinstance(arr, int) or isinstance(arr,DeviceArray):
            pass
        else:
