...
class EncodedNumber(object):
    ...
    def __init__(self, public_key, encoding, exponent, apha=1):
    ...
    def encode(cls, public_key, scalar, precision=None, max_exponent=None):
        ...
        *max_exponent*, and such that the number *precision* is not
        ...
        if max_exponent is None:
            ...
            if isinstance(prec_exponent,DeviceArray):
        ...
        if isinstance(prec_exponent,DeviceArray):
            ...
            encoding_org = int_rep._value % public_key.n
            encoding = jnp.asarray((encoding_org % cls.C_BASE).astype(np.int32))
            apha = encoding_org // cls.C_BASE
            return cls(public_key, encoding, exponent, apha=apha)
        else:
